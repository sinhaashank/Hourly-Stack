...
# creating start and end date for accessing data from the database 
# data is stored in UTC timezone, we need to convert it to the local time --> start date = input_date - 1; end_date = input_date + 1
...
# fucntion to change timezone from UTC to local time
...
df_data = df_merge.loc[(df_merge.index.day==date.day)&(df_merge.index.month==date.month)&(df_merge.index.year==date.year)]
...
def create_plot(
    ...
    fig = plotly.subplots.make_subplots(
        ...
        subplot_titles = (
            ...
            'Cross Border DA Flows: (+)Exports, (-)Imports',
        ),
        shared_xaxes=True,
...
outdir = 'plots/'
outfile = country + '_' + 'Stack' + '_' + str(date.day) + '_' + date.strftime("%B") + '_' + str(date.year) + '.html'
